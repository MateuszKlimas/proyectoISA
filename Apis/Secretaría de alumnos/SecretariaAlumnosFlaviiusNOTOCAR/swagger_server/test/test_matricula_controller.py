...
class TestMatriculaController(BaseTestCase):
    ...
    def test_get_matriculas(self):
        ...
        Devuelve la lista de matrículas que estan asignadas a ese id de asignatura.
        """
        query_string = [('idgradoAsignatura', 56)]
        response = self.client.open('/secretaria-alumnos/getMatriculaidAsignatura',
                                    method='GET',
                                    content_type='application/json',
                                    query_string=query_string)
        self.assert200(response, "Response body is : " + response.data.decode('utf-8'))
