...
class TestAsignaturasController(BaseTestCase):
    ...
    def test_obtener_asignaturas_grado(self):
        """
        Test case for obtener_asignaturas_grado

        Obtiene asignaturas de un grado
        """
        query_string = [('idGrado', 56)]
        response = self.client.open('/Facultad/asignaturasGrado',
                                    method='GET',
                                    content_type='application/json',
                                    query_string=query_string)
        self.assert200(response, "Response body is : " + response.data.decode('utf-8'))
